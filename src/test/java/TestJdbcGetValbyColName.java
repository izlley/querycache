...
public class TestJdbcGetValbyColName {

  static String gQuery = "select * from OCB_BEHAVIOR";
  /*
     static String gQuery = "select mbid from ocb_behavior where 1=1"        +"and (dt between '20140301' and '20140501' "
        +"  or dt = '99991231') " 
        +" and may = 1 " 
        +" and apay = 1 "
        +" and sex = 1 "
        ...
        +" and px between 45.0 - 10.0 and 45.0 + 10.0 "
        +" and py between 13.0 - 10.0 and 13.0 + 10.0 "
        +" and pcl = '20' "
        +" group by mbid "
      ...
      while (rs.next()) {
        for (int i = 1; i <= sColCnt; i++) {
          ...
          switch (type) {
            case Types.VARCHAR:
            case Types.CHAR:
              if (!isSilence) {System.out.print(rs.getString(rsmd.getColumnName(i)));}
              else {rs.getString(i);}
              break;
            case Types.BIGINT:
              if (!isSilence) {System.out.print(rs.getLong(rsmd.getColumnName(i)));}
              else {rs.getLong(i);}
              break;
            case Types.INTEGER:
            case Types.TINYINT:
              if (!isSilence) {System.out.print(rs.getInt(rsmd.getColumnName(i)));}
              else {rs.getInt(i);}
              break;
            case Types.FLOAT:
            case Types.DOUBLE:
              if (!isSilence) {System.out.print(rs.getDouble(rsmd.getColumnName(i)));}
              else {rs.getDouble(i);}
              break;
          }
