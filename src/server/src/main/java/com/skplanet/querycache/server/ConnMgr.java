...
import com.skplanet.querycache.server.common.Types.*;

public class ConnMgr {
  private static final Logger LOG = LoggerFactory.getLogger(ConnMgr.class);
  
  // N.B.: if the list is structurally modified at any time after the iterator is
  ...
  //       2. We need to reduce FreeList periodically to prevent from expanding infinitely
  List<ConnNode> sFreeList =
      Collections.synchronizedList(new LinkedList<ConnNode>());
  // TODO: We need a periodic garbage collection of zombie connections to prevent unintended
  //       UsingMap expansion.
  ConcurrentHashMap<Long, ConnNode> sUsingMap =
      new ConcurrentHashMap<Long, ConnNode>(16, 0.9f, 1);
  ...
  CORE_RESULT initialize(int aInitSize, float aResizingF, ConnType aType) {
    ...
    for (int i = 0; i < aInitSize ;i++) {
      ...
      try {
        ...
        LOG.info("Make " + (i+1) + "st ConnNode for ConnPool.");
  ...
  public ConnNode allocConn(ConnType aType) {
    ...
    try {
      if (sFreeList.size() < sFreeMaxSize * sFreelistThreshold ) {
        ...
        LOG.info("FreeList resizing from " + sFreeMaxSize + " to " + sPostMaxSize);
      }
    ...
    if (sConn != null) {
      ...
      LOG.info("Moving ConnNode from FreeList to UsingMap." +
          "-Free size:" + sFreeList.size() + ",-FreeMaxSize:" + sFreelistMaxSize +
  ...
  public CORE_RESULT closeConn(long aConnId) {
    ...
    sFreeList.add(sConn);
    LOG.info("Moving ConnNode from UsingMap to FreeList." +
        "-Free size:" + sFreeList.size() + ",-FreeMaxSize:" + sFreelistMaxSize +
        ",-Using size:" + sUsingMap.size());
    return CORE_RESULT.CORE_SUCCESS;
  }
  ...
  private String buildUrl(ConnType aType) {
    ...
    switch (aType) {
      case PHOENIX_JDBC:
        LOG.info("Making Phoenix ConnPool.");
        sUrl = "jdbc:phoenix:" + Configure.gPhoenixZkqIp + ":" + Configure.gPhoenixZkPort;
        break;
      case IMPALA_JDBC:
      case IMPALA_THRIFT:
      case HBASE:
